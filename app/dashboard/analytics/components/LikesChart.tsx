...
export function LikesChart({ range, platforms }: ChartProps) {
  ...
  React.useEffect(() => {
    async function load() {
      ...
      const { data: daily } = await supabase
        ...
        .select("date, platform, likes_count")
        .eq("user_id", user.id)
  ...
  const chartData = React.useMemo<ChartRow[]>(() => {
    ...
    filteredData.forEach((row) => {
      ...
      byDate[row.date][row.platform] =
        (byDate[row.date][row.platform] ?? 0) + row.likes;
    });
    ...
    <Card className="pt-0">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          ...
          <CardDescription>
            Daily likes across selected platforms
          </CardDescription>
        </div>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          <AreaChart data={chartData}>
            <defs>
              <linearGradient id="fillTotalLikes" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
